...
class RestApi implements IRestApi {
  ...
  _apiLog(
    ...
    if (!kIsWeb && _showApiLog) {
      ...
      apiCallDetails =
          '${apiCallDetails}HEADERS » ${header ?? dio.options.headers}\n$separator';
      if (body is! FormData && body != {}) {
        apiCallDetails =
            '${apiCallDetails}PAYLOAD » ${LogMaster.prettyJson(body)}\n';
        apiCallDetails = apiCallDetails + separator;
      }
      apiCallDetails =
      ...
      if (!isError) {
        apiCallDetails =
            '${apiCallDetails}STATUS CODE » ${response?.statusCode}\n';
      } else {
        apiCallDetails =
            '${apiCallDetails}ERROR STATUS CODE » ${response?.statusCode}\n';
        apiCallDetails =
            '${apiCallDetails}ERROR STATUS MESSAGE » ${response?.statusMessage}\n';
      }
      LogMaster.info(
        apiCallDetails,
  ...
  Future<Response<dynamic>> _restApi(
    ...
    try {
      ...
      final options = header != null
          ...
          : (attachToken
              ? Options(headers: await getHeader(), responseType: responseType)
              : null);
      switch (apiMethod) {
