    ...
    switch (networkState) {
      case ServiceState.notStarted:
        return initialPlaceHolder ??
            (!isExpanded
                ? const NetworkErrorWidget(text: '')
                : const Scaffold(body: NetworkErrorWidget(text: '')));
      ...
      case ServiceState.error:
        return errorWidget ??
            (!isExpanded
                ...
                : const Scaffold(body: NetworkErrorWidget()));
      case ServiceState.noInternet:
        if (!showNoInternetWarningOnly) {
          return !isExpanded
              ? const Text('No Internet')
              : const NetworkErrorWidget(text: 'NO INTERNET CONNECTION');
        } else {
          return child;
        }
